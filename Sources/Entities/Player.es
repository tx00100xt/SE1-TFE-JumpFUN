...
extern INDEX hud_bShowPing				= FALSE;
extern INDEX hud_bShowKills				= FALSE;
  ...
  if (penKiller!=NULL) {
    ...
    if (IsOfClass(penKiller, "Player")) {
      ...
      if (penKiller!=ppl) {
		...
		_pShell->SetINDEX("rf_cmdSuicide",0);
		_pShell->SetString("rf_cmdUser1",strKillerName);
		_pShell->SetString("rf_cmdUser2",strMyName);
		_pShell->SetString("rf_cmdGun",strGun);
		_pShell->Execute(_pShell->GetString("rf_cmdOnKill"));
      } else {
        // make message from damage type
        switch(eDeath.eLastDamage.dmtType) {
        case DMT_DROWNING:  CPrintF(TRANS("%s drowned\n"), (const char*)strMyName); strGun="Drowned"; break;
        case DMT_BURNING:   CPrintF(TRANS("%s burst into flames\n"), (const char*)strMyName); strGun="Burned"; break;
        case DMT_SPIKESTAB: CPrintF(TRANS("%s fell into a spike-hole\n"), (const char*)strMyName); strGun="Spiked"; break;
        case DMT_FREEZING:  CPrintF(TRANS("%s has frozen\n"), (const char*)strMyName); strGun="Frozen"; break;
        case DMT_ACID:      CPrintF(TRANS("%s dissolved\n"), (const char*)strMyName); strGun="Dissolved"; break;
        case DMT_PROJECTILE:
        case DMT_EXPLOSION:
          CPrintF(TRANS("%s blew himself away\n"), (const char*)strMyName); strGun="Explosion"; break;
        default:            CPrintF(TRANS("%s has committed suicide\n"), (const char*)strMyName); strGun="Other";
        }

		_pShell->SetINDEX("rf_cmdSuicide",1);
		_pShell->SetString("rf_cmdUser1","");
		_pShell->SetString("rf_cmdUser2",strMyName);
		_pShell->SetString("rf_cmdGun",strGun);
		_pShell->Execute(_pShell->GetString("rf_cmdOnKill"));
      }
...
//###########################################################################################################
	...
	void ChangeLight(CEntity *pen, INDEX iNewColor)
	{
		CLightSource lsNew;
		((CLight*)pen)->m_colColor = iNewColor;
		((CLight*)pen)->SetupLightSource(lsNew);
		((CLight*)pen)->m_lsLightSource.SetLightSource(lsNew);
	}

	void AddModel(CTString strModel, CTString strTexture, FLOAT fX, FLOAT fY, FLOAT fZ, FLOAT fAX, FLOAT fAY, FLOAT fAZ, BOOL bColliding, FLOAT fStretch, INDEX iAnimation)
	{
		CModelHolder2 *penTemplateModel = NULL;
		FOREACHINDYNAMICCONTAINER(_pNetwork->ga_World.wo_cenEntities, CEntity, iten) 
		{
			CEntity *pen = iten;
			if(IsDerivedFromClass(pen, "ModelHolder2"))
			{
				penTemplateModel = (CModelHolder2 *)pen;
				break;
			}
		}
		
		if(penTemplateModel!=NULL)
		{
			CModelHolder2 *TestModel	 = (CModelHolder2*)_pNetwork->ga_World.CreateEntity(CPlacement3D(FLOAT3D(fX,fY,fZ),ANGLE3D(fAX,fAY,fAZ)),penTemplateModel->GetClass());
			TestModel->m_fnModel		 = strModel;
			TestModel->m_fnTexture		 = strTexture;
			TestModel->m_bColliding		 = bColliding;
			TestModel->m_iModelAnimation = iAnimation;
			TestModel->m_fStretchAll	 = fStretch;
			TestModel->InitModelHolder();
			TestModel->StretchModel();
			_pNetwork->ga_World.RebuildLinks();
		} else { CPrintF("No model found ???\n"); }
   }
...
//######################                       ST8VI MOD                          ###########################	
//###########################################################################################################	
	INDEX _st8_bEnableRegeneration = GetSP()->sp_st8_bEnableRegeneration;
	FLOAT _st8_fRegeneration = GetSP()->sp_st8_fRegeneration;
	// if _st8_bEnableRegeneration == TRUE
	if (_st8_bEnableRegeneration == TRUE && GetFlags()&ENF_ALIVE) {
      // slowly increase health with time
      FLOAT fHealth = GetHealth();
      FLOAT fTopHealth = TopHealth();
      if (fHealth<fTopHealth) {
        _st8_fRegeneration = Clamp(_st8_fRegeneration, (float)1.0f, (float)5.0f);		
		SetHealth(ClampUp(fHealth + _pTimer->TickQuantum*_st8_fRegeneration, fTopHealth));  // one unit per second
	  }
    }
//###########################################################################################################	
//###########################################################################################################	
...
//######################                       ST8VI MOD                          ###########################	
//###########################################################################################################
	INDEX _st8_bEnableExtGravity = GetSP()->sp_st8_bEnableExtGravity;
	FLOAT _st8_fDamageAmount = GetSP()->sp_st8_fDamageAmount;
	FLOAT _st8_fDirection = GetSP()->sp_st8_fDirection;
	FLOAT _st8_fGravityDir = GetSP()->sp_st8_fGravityDir;	
	if(_st8_bEnableExtGravity){ 
		if( penInflictor == this && dmtType == DMT_EXPLOSION) { 
		
			_st8_fDamageAmount = Clamp(_st8_fDamageAmount, (float)0.1f, (float)0.9f);;
			//fDamageAmmount *= 0.2f; 
			fDamageAmmount *= _st8_fDamageAmount;
		
			_st8_fDirection = Clamp(_st8_fDirection, (float)0.05f, (float)7.5f);
			_st8_fGravityDir = Clamp(_st8_fGravityDir, (float)5.0f, (float)50.0f);
			//GiveImpulseTranslationAbsolute(vDirection*(fDamageAmmount/1.5f)-en_vGravityDir*(fDamageAmmount/30));
			GiveImpulseTranslationAbsolute(vDirection*(fDamageAmmount/_st8_fDirection)-en_vGravityDir*(fDamageAmmount/_st8_fGravityDir));
		}
	}
...
//######################                       ST8VI MOD                          ###########################	
//###########################################################################################################
	INDEX _st8_bEnableExtSpeed = GetSP()->sp_st8_bEnableExtSpeed;
	FLOAT _st8_fPlayerExtSpeed = GetSP()->sp_st8_fPlayerExtSpeed;	
	if(_st8_bEnableExtSpeed) { 
        if(_st8_fPlayerExtSpeed > 2.0f) {_st8_fPlayerExtSpeed = 2.0f;}
		if(_st8_fPlayerExtSpeed < 0.9f) {_st8_fPlayerExtSpeed = 0.9f;}
		//vTranslation *= _st8_fPlayerExtSpeed; // full with jump
		vTranslation(1) *= _st8_fPlayerExtSpeed; // only
		vTranslation(3) *= _st8_fPlayerExtSpeed; // speed
    }
